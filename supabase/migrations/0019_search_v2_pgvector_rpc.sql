-- In-DB candidate retrieval: filter + vector-KNN in Postgres so search never loads the
-- whole index into Node memory (the cold-start / scale bottleneck). Returns a bounded
-- candidate pool; the app does the cheap refine (flavour, avoid scan, rank, buckets).
create or replace function search_v2_candidates(
  p_query_embedding vector(1024),
  p_type_embedding  vector(1024) default null,
  p_type_exact      text[]       default null,   -- exact primary_type matches (type + synonyms)
  p_type_threshold  float        default 0.15,   -- cosine-distance cap for type_embedding match (=1-0.85)
...
language sql stable as $$
  ...
  where pi.data_quality_score >= p_min_quality
    ...
    and (
      -- type gate: no type wanted, exact primary_type match, or close type-embedding
    ...
    -- dietary: keep unknown (null) per the missing-data policy; only exclude explicit false
    ...
    and (p_query_embedding is null or pi.embedding is not null)
  order by case when p_query_embedding is null then 0
