/** Catalog-derived + curated tokens for local intent classification (no network). */
...
export function buildProductTypeSet(subcategories?: Iterable<string>): Set<string> {
  ...
  for (const raw of subcategories ?? []) {
    ...
    if (!s) continue;
    for (const part of s.split(/[^a-z0-9]+/i)) {
      const t = part.trim();
      if (t.length >= 3 && !STOP_TOKENS.has(t) && !/^\d+$/.test(t)) out.add(t);
    }
  }
  return out;
}
...
export function resolveIntentSignals(opts?: IntentSignalOpts): {
  brands: Set<string> | null;
  productTypes: Set<string>;
} {
  return {
    brands: normalizeBrandSet(opts?.brands),
    productTypes: opts?.productTypes ?? buildProductTypeSet(opts?.subcategories),
  };
}
